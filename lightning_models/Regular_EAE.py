...
def select_class(data,classname):
    filter_data = []
    for image,label in data:
        if label == classname:
            filter_data.append((image,label))
...
class Regular_EAE(LightningModule):
    def __init__(self,laten_dims: int, classname:int, lr=0.0005248):
        super(Regular_EAE, self).__init__()
...
# 			
    ...
    def setup(self, stage):
        ...
        cifar_test = torchvision.datasets.CIFAR10(root='./datasets',train=False, download=True, transform=transform)

        subclass = select_class(cifar_train,self.classname)
        subclass_test = select_class(cifar_test,self.classname)
        # train/val split
        cifar_train2, cifar_val =  torch.utils.data.random_split(subclass, [4500, 500])

        # assign to use in dataloaders
        self.train_dataset = cifar_train2
        self.val_dataset = cifar_val
        self.test_dataset = subclass_test
...
def main(): 
    logger = TensorBoardLogger("lightning", name="Regular_EAE",log_graph=True)

    latent_dims = 64
    for cla in range(10):
        model = Regular_EAE(latent_dims,cla)
